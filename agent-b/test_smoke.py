"""
agent-b/test_smoke.py
---------------------
Smoke tests for agent-b — validate config loading, storage read/write,
...
# Ensure the agent-b package directory is on the path
sys.path.insert(0, os.path.dirname(__file__))

# Set env vars BEFORE importing any modules that read them at import time
os.environ["OPENAI_API_KEY"] = "test-openai-key"
os.environ["STRATEGIES_DIR"] = "/tmp/test_strategies_b"
...
def test_config_loads():
    ...
    assert cfg.openai_api_key == "test-openai-key", (
        f"Expected 'test-openai-key', got '{cfg.openai_api_key}'"
    )
    assert cfg.strategies_dir == "/tmp/test_strategies_b", (
        f"Expected '/tmp/test_strategies_b', got '{cfg.strategies_dir}'"
    )
    assert cfg.agent_b_cron == "0 6 * * 1", (
        f"Expected '0 6 * * 1', got '{cfg.agent_b_cron}'"
    )
...
def test_storage_write_read():
    ...
    test_dir = "/tmp/test_strategies_b"
    # Clean slate
    ...
    content = "# Test Strategy GPT\n\nThis is a test.\n"
    filename = "strategy_gpt_test_2024-01.md"
